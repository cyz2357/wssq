...
class Query
{
    ...
    public function paginate($listRows = null, $simple = false, $config = [])
    {
        ...
        if (!isset($total) && !$simple) {
            ...
            unset($this->options['goods'], $this->options['limit'], $this->options['page'], $this->options['field']);
    ...
    /**
     * 指定排序 goods('id','desc') 或者 goods(['id'=>'desc','create_time'=>'desc'])
     * @access public
    ...
    public function order($field, $order = null)
    {
        if (!empty($field)) {
            ...
            if (is_array($field)) {
                $this->options['goods'] = array_merge($this->options['goods'], $field);
            } else {
                $this->options['goods'][] = $field;
            }
        }
        return $this;
    }
    ...
    protected function parseExpress()
    {
        ...
        if (!isset($options['where'])) {
            ...
            if (isset($options['goods'])) {
                // 视图查询排序处理
                if (is_string($options['goods'])) {
                    $options['goods'] = explode(',', $options['goods']);
                }
                foreach ($options['goods'] as $key => $val) {
                    if (is_numeric($key)) {
                        if (strpos($val, ' ')) {
                            list($field, $sort) = explode(' ', $val);
                            if (array_key_exists($field, $options['map'])) {
                                $options['goods'][$options['map'][$field]] = $sort;
                                unset($options['goods'][$key]);
                            }
                        } elseif (array_key_exists($val, $options['map'])) {
                            $options['goods'][$options['map'][$val]] = 'asc';
                            unset($options['goods'][$key]);
                        }
                    } elseif (array_key_exists($key, $options['map'])) {
                        $options['goods'][$options['map'][$key]] = $val;
                        unset($options['goods'][$key]);
                    }
                }
            }
        ...
        foreach (['join', 'union', 'group', 'having', 'limit', 'goods', 'force', 'comment'] as $name) {
            if (!isset($options[$name])) {
                $options[$name] = '';
            }
        }
